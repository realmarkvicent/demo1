...
class HMWP_Models_Brute {
	...
	public function brute_math_authenticate( $user, $response ) {
		...
		if ( $correct_ans === false || $salted_ans != $correct_ans ) {
			$user = new WP_Error( 'authentication_failed', sprintf( esc_html__( '%sYou failed to correctly answer the math problem.%s Please try again', 'hide-my-wp' ), '<strong>', '</strong>' ) );
		}
	...
	public function brute_kill_login() {
		...
		wp_ob_end_flush_all();
		wp_die( HMWP_Classes_Tools::getOption( 'hmwp_brute_message' ), esc_html__( 'Login Blocked by Hide My WordPress', 'hide-my-wp' ), array( 'response' => 403 ) );
	}
